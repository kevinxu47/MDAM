...
MDAM_sys = function(sub_dat, T_X1 = sum(population$X1), T_X2 = sum(population$X2), L = 10){
  ...
  for(i in 1:L){
    ...
    # Randomly set working probablity for X1
    # adj_dat_U_0$X1_init_prob <- runif(nrow(adj_dat_U_0))
    # adj_dat_U_1$X1_init_prob <- runif(nrow(adj_dat_U_1))
    ...
    # weighted sum of X2i = 1 | X1i = 1, Ui = 0
    w_x2_1_given_x1_1 <- sum(adj_dat[adj_dat$U==0 & adj_dat$X1 == 1 & adj_dat$X2 == 1,]$W)
    # weighted sum of X2i = 0 | X1i = 1, Ui = 0
    w_x2_0_given_x1_1 <- sum(adj_dat[adj_dat$U==0 & adj_dat$X1 == 1 & adj_dat$X2 == 0,]$W)  
    # weighted sum of X2i = 1 | X1i = 0, Ui = 0
    w_x2_1_given_x1_0 <- sum(adj_dat[adj_dat$U==0 & adj_dat$X1 == 0 & adj_dat$X2 == 1,]$W)
    # weighted sum of X2i = 0 | X1i = 0, Ui = 0
    w_x2_0_given_x1_0 <- sum(adj_dat[adj_dat$U==0 & adj_dat$X1 == 0 & adj_dat$X2 == 0,]$W)
    
    # P(X2 = 1 | X1 = 0, U = 1) is calculated by fixing log odd ratio from U = 0
    beta1_U_0 = log(w_x2_1_given_x1_1 / w_x2_0_given_x1_1) - log(w_x2_1_given_x1_0 / w_x2_0_given_x1_0)
    beta0_U_0 = log(w_x2_1_given_x1_0 / w_x2_0_given_x1_0)
    ...
    # Find expectation of weight sum of X2 for U = 1 (given X1 = 1 and X1 = 0, respectively)
    EX2_X1_1 = logit_inv(beta0_U_1 + beta1_U_1) * sum(adj_dat[adj_dat$U==1 & adj_dat$X1 == 1,]$W)
    EX2_X1_0 = logit_inv(beta0_U_1) * sum(adj_dat[adj_dat$U==1 & adj_dat$X1 == 0,]$W)
    ...
    # Randomly set working probablity for X2
    # adj_dat_U_0$X2_init_prob <- runif(nrow(adj_dat_U_0))
    # adj_dat_U_1$X2_init_prob <- runif(nrow(adj_dat_U_1))
    
    # Merge them back
    adj_dat <- rbind(adj_dat_U_0, adj_dat_U_1)
    
    f_2c_1 = EX2_X1_1 / sum(adj_dat[adj_dat$U == 1 & adj_dat$X1 == 1, ]$W * adj_dat[adj_dat$U == 1 & adj_dat$X1 == 1, ]$X2_init_prob)
    X2_prob_X1_1 = f_2c_1 * adj_dat_U_1[adj_dat_U_1$X1 == 1,]$X2_init_prob
    
    f_2c_0 = EX2_X1_0 / sum(adj_dat[adj_dat$U == 1 & adj_dat$X1 == 0, ]$W * adj_dat[adj_dat$U == 1 & adj_dat$X1 == 0, ]$X2_init_prob)
    X2_prob_X1_0 = f_2c_0 * adj_dat_U_1[adj_dat_U_1$X1 == 0,]$X2_init_prob
    
    # Replaces values less than 0 with 0, then replaces values greater than 1 with 1.
    X2_prob_X1_1 <- pmin(pmax(X2_prob_X1_1, 0), 1)
    X2_prob_X1_0 <- pmin(pmax(X2_prob_X1_0, 0), 1)
...
for(i in 1:length(SAMPLE)){
  RESULTS[[i]] = MDAM_sys(sub_dat=SAMPLE[[i]])
}
