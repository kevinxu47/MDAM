...
IH = function(sub_dat, T_X1 = sum(population$X1), T_X2 = sum(population$X2), L = 10){
  ...
  for(i in 1:L){
    ...
    ###### hot deck imputation ###### 
    # Cell for selection
    cell_selection = which(adj_dat$U==0)
    
    # impute unit nonrespondents for X1, X2, Y1, Y2, Y3, Y4
    rows_to_impute = which(adj_dat$U==1)
    
    for (k in rows_to_impute) {
      adj_dat[k, c("X1", "X2", "Y1", "Y2", "Y3", "Y4")] = 
        adj_dat[sample(cell_selection, 1), c("X1", "X2", "Y1", "Y2", "Y3", "Y4")]
    }
