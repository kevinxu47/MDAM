...
# Convert factors into integers for X1, X2, Y1 and Y2
population$X1 <- as.integer(as.character(population$X1))
population$X2 <- as.integer(as.character(population$X2))
population$Y1 <- as.integer(as.character(population$Y1))
population$Y2 <- as.integer(as.character(population$Y2))

# Convert factors into integers for X1, X2, Y1 and Y2 for SAMPLE
SAMPLE <- lapply(SAMPLE, function(df) {
  df$X1 <- as.integer(as.character(df$X1))
  df$X2 <- as.integer(as.character(df$X2))
  df$Y1 <- as.integer(as.character(df$Y1))
  df$Y2 <- as.integer(as.character(df$Y2))
  return(df)
})


# MDAM-vary imputation
MMH = function(sub_dat, pop_X1_mean = sum(population$X1), pop_X2_mean = sum(population$X2), L = 10){
