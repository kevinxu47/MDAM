...
MDAM_vary = function(sub_dat, T_X1 = sum(population$X1), T_X2 = sum(population$X2), L = 10){
  ...
  for(i in 1:L){
    ...
    fit <- glm(X1 ~ W, data = adj_dat_U_0, family = binomial)
    
    # Predicted probabilities for each observation
    adj_dat_U_0$X1_init_prob  <- predict(fit, type = "response")
    
    # Predict probabilities for adj_dat_U_1 using the fitted model
    adj_dat_U_1$X1_init_prob <- predict(fit, newdata = adj_dat_U_1, type = "response")
    ...
    # Normalize the probability vector if any value is greater than 1
    if (any(X1_prob > 1)) {
      X1_prob <- X1_prob / sum(X1_prob)
    }
    
    adj_dat$X1[which(adj_dat$U == 1)] = rbinom(n_unit1, 1, prob = X1_prob)
    adj_dat$X1_prob_final = X1_prob
...
for(i in 1:length(SAMPLE)){
  RESULTS[[i]] = MDAM_vary(sub_dat=SAMPLE[[i]])
}
